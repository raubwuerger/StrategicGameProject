...
LogMessage::LogMessage( const unsigned long index, const Time& time, const LogLevel& logLevel, const std::string& message, const std::string& category )
 :	m_Index(index),
	m_Time(time),
	m_LogLevel(&logLevel),
	m_Message(message),
...
const std::string& LogMessage::GetCategory() const
{
	return m_Category;
}
...
const Time& LogMessage::GetLogTime() const
{
	return m_Time;
}
